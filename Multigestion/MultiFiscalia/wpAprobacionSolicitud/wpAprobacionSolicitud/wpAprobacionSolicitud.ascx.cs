...
namespace MultiFiscalia.wpAprobacionSolicitud.wpAprobacionSolicitud
{
    ...
    public partial class wpAprobacionSolicitud : WebPart
    {
        ...
        protected void btnAprobar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dtArchivosCriticos.Rows.Count > 0)
                {
                    ...
                    dtArchivos = new Documentos { }.buscarArchivos(lbEmpresa.Text.Trim(), lbRut.Text.Trim(), objresumen.desEtapa.Trim(), objresumen.idOperacion.ToString());
            ...
            try
            {
                ...
                if ((objresumen.desEtapa.ToLower() == "fiscalia") && tbServiciosGarantia1.Rows.Count > 0 && tbServiciosOperaciones1.Rows.Count > 0)
                {
                    if (ViewState["ACCION"].ToString() == Constantes.OPCION.INSERTAR)
                    {
                        ...
                        exito = Ln.InsertarSolicitudFiscalia(objresumen.idEmpresa.ToString(), objresumen.idOperacion.ToString(), xmlEmpres, xmlGarantia, txtComentarios.Text.ToString(), txtComentariosFiscalia.Text.ToString(), objresumen.idUsuario.ToString(), objresumen.descCargo.ToString(), validacionServicio);
                        if (exito)
                        {
                            ocultarDiv();
                            MensajeOk(Ln.buscarMensaje(Constantes.MENSAJE.EXITOINSERT));

                            if (Ln.InsertarActualizarEstados(objresumen.idEmpresa.ToString(), objresumen.idOperacion.ToString(), objresumen.idUsuario, objresumen.descCargo, "6", "Fiscalia", "18", "Confección Contratos", "1", "Ingresado", "Fiscalia", txtComentarios.Text.ToString()))
                            {
                                Ln.ActualizarEstado(objresumen.idOperacion.ToString(), "6", "Fiscalia", "18", "Confección Contratos", "1", "Ingresado", "Fiscalia"); //ACTUALIZA ESTADO LISTA OPERACIONES
                ...
                {
                    ocultarDiv();
                    MensajeError(Ln.buscarMensaje(Constantes.MENSAJE.FISCINCOMPLETOS));
                }
            }
            catch (Exception ex)
            {
        ...
        protected void mostrarDatos()
        {
            ...
            res = Ln.ListarResumenTotal(objresumen.idEmpresa.ToString(), objresumen.idPAF.ToString(), objresumen.idOperacion.ToString());//agregarle el idGarantia
        ...
        private void ValidarDocCriticos(int valEmpresa, int valGarantia)
        {
            ...
            dtArchivosEnShare = new Documentos { }.buscarArchivos(lbEmpresa.Text.Trim(), lbRut.Text.Trim(), "FISCALIA", objresumen.idOperacion.ToString());
        ...
        {
            LogicaNegocio Ln = new LogicaNegocio();
            DataTable dtServiciosOperacion = Ln.ListarServiciosOperacion();
            util.CargaDDL(ddlServicioEmpresa, dtServiciosOperacion, "Nombre", "Id");
        }

        private void garantias()
        {
            LogicaNegocio Ln = new LogicaNegocio();
            DataTable dtServiciosGarantia = Ln.ListarServiciosGarantia(1);
            util.CargaDDL(ddlServiciosGarantia, dtServiciosGarantia, "Nombre", "Id");
        }
        /// <summary>
        /// FiscaliaCostos
        /// </summary>
