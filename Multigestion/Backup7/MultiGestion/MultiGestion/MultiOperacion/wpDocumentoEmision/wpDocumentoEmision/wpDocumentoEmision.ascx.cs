...
namespace MultiOperacion.wpDocumentoEmision.wpDocumentoEmision
{
    ...
    public partial class wpDocumentoEmision : WebPart
    {
        ...
        private void CargarTipoDocumento()
        {
            ...
            dt = MTO.ConsultarDocumentosParametrizados(Convert.ToInt32(ViewState["IdProducto"]), Convert.ToInt32(ViewState["IdAcreedor"]), "Operaciones");
        ...
        protected void gridDocumentosCurse_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            {
                //LogicaNegocio MTO = new LogicaNegocio();
                string nombre = util.ObtenerNombreArchivo(e.CommandArgument.ToString(), ViewState["Certificado"].ToString(), objresumen);
                if (util.EliminarArchivo(nombre, ViewState["Certificado"].ToString(), objresumen))
                {
            ...
            if (e.CommandName == "Descargar")
            {
                ...
                {
                    datos["ValorUF"] = indicador.Substring(indicador.IndexOf("/") + 1);
                    datos["ValorUS"] = indicador.Substring(0, indicador.IndexOf("/"));
                }
                else
                {
                    datos["ValorUF"] = "0";
                ...
                {
                    byte[] archivo = MTO.GenerarReporte("Solicitud_de_Emision", datos, "admin", objresumen);
                    if (archivo != null)
                    {
                ...
                {
                    byte[] archivo = MTO.GenerarReporte("Instruccion_de_Curse", datos, "admin", objresumen);
                    if (archivo != null)
                    {
                        Page.Session["tipoDoc"] = "pdf";
                        Page.Session["binaryData"] = archivo;
                ...
                {
                    dt = MTO.ListarPlantillaById(Convert.ToInt32(idPlantilla));

                    if (dt.Rows.Count > 0)
                    {
                        ocultarDiv();
                        DataSet dtLista = new DataSet();
                        dtLista = MTO.CargarDatosHtml(Convert.ToInt32(objresumen.idOperacion.ToString()));
