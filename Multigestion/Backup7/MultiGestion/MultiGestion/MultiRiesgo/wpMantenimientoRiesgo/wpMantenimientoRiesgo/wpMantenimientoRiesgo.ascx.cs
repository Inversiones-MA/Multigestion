    ...
    public partial class wpMantenimientoRiesgo : WebPart
    {
        ...
        protected void CargarGrid()
        {
            try
            {
                ...
                res = MTO.ListarRiesgo(razonSocial, rut, descEjecutivo, ViewState["CARGO"].ToString(), ViewState["AREA"].ToString());
        ...
        protected void ResultadosBusqueda_Command1(object sender, CommandEventArgs e)
        {
            ...
            for (int i = 0; i <= dt.Rows.Count - 1; i++)
            {
                if (dt.Rows[i]["Opcion"].ToString() == e.CommandName)
                {
                    try
                    {
                        MultigestionUtilidades.Resumen resumenP = new MultigestionUtilidades.Resumen();
                        ...
                        //resumenP.fecInicioActEconomica = DateTime.Parse(ResultadosBusqueda.Rows[index].Cells[2].Text.ToString());
        ...
        protected void ResultadosBusqueda_RowCreated(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.Cells.Count > 2)
            {
                ...
                {
                    e.Row.Cells[23].Visible = false; //Accion
                }
            }
        }
