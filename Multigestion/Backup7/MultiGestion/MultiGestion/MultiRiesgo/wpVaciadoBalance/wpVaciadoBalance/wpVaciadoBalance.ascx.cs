    ...
    {     
        ...
        protected void ResultadosBusqueda_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.RowType == DataControlRowType.Header)
            {
                ...
                MTO.CargarListaVaciadoBalance(ref cmbTipoDocumentAux);
        ...
        protected void btnFinalizar_Click(object sender, EventArgs e)
        {
            ...
            if (mensaje == string.Empty)//MENSAJE ME INDICA QUE HACE FALTA GUARDAR ALGUNOS AÑOS
            {
                ...
                {
                    exito = MTO.finalizarBalance(objresumen.idEmpresa.ToString(), System.Web.HttpUtility.HtmlDecode(ResultadosBusqueda.Rows[0].Cells[14].Text.ToString()).Trim(), xmlData, Constantes.RIESGO.VALIDACION);
                    pnFormulario.Enabled = false;
                    if (exito)
                    {
                        ocultarDiv();
                ...
                else
                {
                    ...
                    exito = MTO.InsertarFinalizarBalance(objresumen.idEmpresa.ToString(), xmlData);
                    pnFormulario.Enabled = false;
                    if (exito)
                    {
                        ocultarDiv();
        ...
        private void llenargrid()
        {
            ...
            LogicaNegocio MTO = new LogicaNegocio();
            DataTable res;
            res = MTO.ListarBalance(objresumen.idEmpresa.ToString());
            int con = 0;
        ...
        private void guardar()
        {
            ...
            if ((auditado && ComentarioExiste) || (!auditado))
            {
                if (permiteGuardar)
                {
                    ...
                    {
                        exito = MTO.InsertarBalance(objresumen.idEmpresa.ToString(), xmlDatos, objresumen.idUsuario, txtComentario.Text);
                        if (exito)
                        {
                            ocultarDiv();
                            dvSuccess.Style.Add("display", "block");
                    ...
                    {
                        exito = MTO.ModificarBalance(objresumen.idEmpresa.ToString(), System.Web.HttpUtility.HtmlDecode(ResultadosBusqueda.Rows[0].Cells[14].Text.ToString()).Trim(), xmlDatos, objresumen.idUsuario, txtComentario.Text);
                        if (exito)
                        {
                            ocultarDiv();
                            dvSuccess.Style.Add("display", "block");
        ...
        public byte[] GenerarReporte(int idEmpresa)
        {
            ...
            try
            {
                ...
                res1 = MTO.ConsultaReporteVaciadoBalance(objresumen.idEmpresa);
