    ...
    public partial class wpListarOperaciones : WebPart
    {
            ...
            for (int i = 0; i <= dt.Rows.Count - 1; i++)
            {
                if (dt.Rows[i]["Opcion"].ToString() == e.CommandName)
                {
                    try
                    {
                        ...
                        resumenP.linkPrincial = "ListarOperaciones.aspx"; //resumenP.linkError = "Mensaje.aspx";
                        ...
                        resumenP.idEtapa = int.Parse(ResultadosBusqueda.Rows[index].Cells[27].Text.ToString());
                        resumenP.idSubEtapa = int.Parse(ResultadosBusqueda.Rows[index].Cells[28].Text.ToString());
                        resumenP.idEstado = int.Parse(ResultadosBusqueda.Rows[index].Cells[29].Text.ToString());
                        SPWeb app2 = SPContext.Current.Web;
        ...
        protected void ResultadosBusqueda_RowCreated(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.Cells.Count > 2)
            {
                ...
                e.Row.Cells[28].Visible = false; //IdSubEtapa
                e.Row.Cells[29].Visible = false; //IdEstado
            }
        }
        ...
        protected void CargarGrid(string etapa, string subEtapa, string estado, string buscar, int pageS, int pageN)
        {
            try
            {
                ...
                LogicaNegocio Ln = new LogicaNegocio();
                DataSet res;
                res = Ln.ListarOperaciones(etapa, subEtapa, estado, buscar, ViewState["AREA"].ToString(), ViewState["CARGO"].ToString(), USER.ToString(), pageS, pageN);
