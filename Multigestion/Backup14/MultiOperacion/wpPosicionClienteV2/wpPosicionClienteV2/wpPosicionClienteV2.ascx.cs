...
namespace MultiOperacion.wpPosicionClienteV2.wpPosicionClienteV2
{
    ...
    public partial class wpPosicionClienteV2 : WebPart
    {
        ...
        protected void gridContactos_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            ...
            LogicaNegocio Ln = new LogicaNegocio();
            DataSet res;
            res = Ln.ConsultaPosicionCliente(Page.Session["IdEmpresa"].ToString()); gridContactos.DataSource = res.Tables[5];
        ...
        private void SumarGarantias(DataTable dt)
        {
            if (dt.Rows.Count > 0)
            {
                ...
                decimal totalValorAjustado = dt.AsEnumerable().Where(r => r.Field<string>("Estado").Contains("Constituida") || r.Field<string>("Estado").Contains("Tramite")).Sum(row => row.Field<decimal>("Valor Ajustable UF"));
                //decimal totalValorAjustado1 = dt.AsEnumerable().Sum(row => row.Field<decimal>("Valor Ajustable UF"));
                decimal totalValorComercial = dt.AsEnumerable().Where(r => r.Field<string>("Estado").Contains("Constituida") || r.Field<string>("Estado").Contains("Tramite")).Sum(row => row.Field<decimal>("Valor Comercial UF"));
                    //dt.AsEnumerable().Sum(row => row.Field<decimal>("Valor Comercial UF"));
                decimal totalValorsegurable = dt.AsEnumerable().Where(r => r.Field<string>("Estado").Contains("Constituida") || r.Field<string>("Estado").Contains("Tramite")).Sum(row => row.Field<decimal>("Valor Asegurable UF"));
                    //dt.AsEnumerable().Sum(row => row.Field<decimal>("Valor Asegurable UF"));
        ...
        private void imprimir()
        {
            ...
            byte[] archivo = new Reportes { }.GenerarReportePosicionCliente(Reporte, datos, "");
            if (archivo != null)
            {
