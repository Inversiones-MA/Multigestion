...
namespace MultiComercial.wpClientes.wpClientes
{
    [ToolboxItemAttribute(false)]
    public partial class wpClientes : WebPart
    {
        ...
        private static string pagina = "EmpresaClientes.aspx";
            ...
            if (dt.Rows.Count > 0)
            {
                if (!Page.IsPostBack)
                {
                    ...
                    if (Page.Session["RESUMEN"] != null)
                    {
                        ...
                        string UsuarioFormulario = vem.verificacionEdicionSimultanea(objresumen.idUsuario, objresumen.idEmpresa, objresumen.idOperacion, "EmpresaClientes", "Empresa");
        ...
        protected void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    if (Ln.InsertarClientexEmpresa(objresumen.idEmpresa, txt_nombre.Text, txt_rut.Text, txt_divRut.Text, txt_porcVentas.Text.Replace(",", "."), txt_plazo.Text, objresumen.idUsuario))
                    {
                        dvSuccess.Style.Add("display", "block");
                        lbSuccess.Text = "Se ha insertado el cliente con éxito";
                        LimpiarCampos();
                    }
                    else
                    {
                        dvError.Style.Add("display", "block");
                        lbError.Text = "El cliente no se ha insertado correctamente";
                    }
                }
                if (ViewState["ACCION"].ToString() == Constantes.OPCION.MODIFICAR)
                {
                    if (Ln.ModificarClientexEmpresa(int.Parse(hdnIdEmpresaClientes.Value), txt_nombre.Text, txt_rut.Text, txt_divRut.Text, txt_porcVentas.Text.Replace(",", "."), txt_plazo.Text, objresumen.idUsuario))
                    {
                        dvSuccess.Style.Add("display", "block");
                        lbSuccess.Text = "Se ha modificado el cliente correctamente";
                        LimpiarCampos();
                    }
                    else
                    {
                        dvError.Style.Add("display", "block");
                        lbError.Text = "No se ha completado la modificación del cliente";
                    }
                }
            }
            catch (Exception ex)
        ...
        protected void ResultadosBusqueda_Command(object sender, CommandEventArgs e)
        {
            ...
            {
                int idEmpresaCliente = Convert.ToInt32(e.CommandArgument.ToString());
                if (Ln.EliminarClientexEmpresa(idEmpresaCliente, objresumen.idUsuario))
                {
                    dvSuccess.Style.Add("display", "block");
                    lbSuccess.Text = "Se ha eliminado correctamente el cliente";
                }
                else
                {
                    dvError.Style.Add("display", "block");
                    lbError.Text = "No se ha eliminado el cliente. Contáctese con el administrador del sitio";
                }
                llenargrid();
                //LimpiarCampos();
            }
            ...
            if (e.CommandName == "Editar")
            {
                ...
                hdnIdEmpresaClientes.Value = ResultadosBusqueda.Rows[index].Cells[0].Text.ToString();
                txt_nombre.Focus();
            }
        }
        ...
        private void asignacionesJS()
        {
            ...
            btnGuardar.OnClientClick = "return validarEmpresaClientes('" + btnGuardar.ClientID.Substring(0, btnGuardar.ClientID.Length - 10) + "');";
        }
        ...
        private void llenargrid()
        {
            ...
            try
            {
                LogicaNegocio Ln = new LogicaNegocio();
                DataTable res;
                res = Ln.ListarClientesxEmpresa(objresumen.idEmpresa);
        ...
        private void LimpiarCampos()
        {
            ...
            hdnIdEmpresaClientes.Value = String.Empty;
        }
