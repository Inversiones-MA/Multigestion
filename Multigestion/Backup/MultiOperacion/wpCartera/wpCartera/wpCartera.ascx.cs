...
namespace MultiOperacion.wpCartera.wpCartera
{
    ...
    public partial class wpCartera : WebPart
    {
        ...
        protected void gridSeguimiento_Command1(object sender, CommandEventArgs e)
        {
            ...
            for (int i = 0; i <= dt.Rows.Count - 1; i++)
            {
                ...
                if ("Prepagar" == e.CommandName)
                {
                    ...
                    //res1 = Ln.ListarDatosCartera(ddlAcreedor.SelectedValue, ddlEdoCertificado.SelectedValue, ddlEjecutivo.SelectedItem.ToString(), USER, ViewState["CARGO"].ToString(), txtNCertificado.Text, txtRUT.Text.Replace(".", ""), txtRazonSocial.Text, ViewState["AREA"].ToString(), ddlFondo.SelectedValue);
        ...
        {
            LogicaNegocio Ln = new LogicaNegocio();
            DataTable dtEstCertificado = Ln.ListarEstadoCertificado();
            util.CargaDDL(ddlEdoCertificado, dtEstCertificado, "Nombre", "Id");
        }
