...
namespace MultiFiscalia.wpServiciosLegalesGarantia.wpServiciosLegalesGarantia
{
    ...
    public partial class wpServiciosLegalesGarantia : WebPart
    {
        ...
        {
            LogicaNegocio Ln = new LogicaNegocio();
            var dt = Ln.ListarServiciosGarantia(1);
            util.CargaDDL(ddlTipo, dt, "Nombre", "ID");
        }
        ...
        public void guardarArchivo()
        {
            ...
            string carpetaEmpresa = util.carpetaEmpresa(lbEmpresa.Text.Trim(), lbRut.Text.Trim());
            new Documentos { }.CargarDocFiscalia(carpetaEmpresa, objresumen.area, fileDocumento, objresumen.idEmpresa.ToString(), ddlTipo.SelectedItem.Text, lbOperacion.Text.Trim(), "Servicios Legales Garntía", Path.GetExtension(fileDocumento.FileName).ToLower());
        }
