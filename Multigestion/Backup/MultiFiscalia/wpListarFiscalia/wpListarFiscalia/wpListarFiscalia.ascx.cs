...
namespace MultiFiscalia.wpListarFiscalia.wpListarFiscalia
{
    ...
    public partial class wpListarFiscalia : WebPart
    {
            ...
            try
            {
                ...
                if (e.Row.RowIndex > -1)
                {
                    ...
                    for (int i = 0; i <= dt.Rows.Count - 1; i++)
                    {
                        ...
                        if (int.Parse(System.Web.HttpUtility.HtmlDecode((e.Row.Cells[11].Text.Trim()))) < 2 && dt.Rows[i]["Opcion"].ToString() == "ServicioFiscaliaEmpresa")
                        {
                            lbmenu[i].Enabled = false;
                            lbmenu[i].OnClientClick = "return false;";
                        }
                        if (int.Parse(System.Web.HttpUtility.HtmlDecode((e.Row.Cells[12].Text.Trim()))) < 2 && dt.Rows[i]["Opcion"].ToString() == "ServicioFiscaliaGarantia")
                        {
                            lbmenu[i].Enabled = false;
                            lbmenu[i].OnClientClick = "return false;";
                        }
                        ...
                        {
                            if (e.Row.Cells[32].Text.Trim() == "&nbsp;")
                                lbmenu[i].Visible = false;
                            else if (int.Parse(System.Web.HttpUtility.HtmlDecode(e.Row.Cells[32].Text.Trim())) == 1 && dt.Rows[i]["Opcion"].ToString() == "Devolver Operacion Flujo Fiscalia")
                                lbmenu[i].Visible = false;                            
                            else if (int.Parse(System.Web.HttpUtility.HtmlDecode((e.Row.Cells[32].Text.Trim()))) > 1 && dt.Rows[i]["Opcion"].ToString() == "DevolucionOperacion")
                                lbmenu[i].Visible = false;                          
                        }
        ...
        protected void ddlUsuariosFiscalia_Selected(string idoperacion, string usuarioF)
        {
            ...
            {
                LogicaNegocio Ln = new LogicaNegocio();
                Ln.GestionResponsableOperacion(int.Parse(idoperacion), usuarioF, ViewState["CARGO"].ToString(), "02");
                CargarGrid(ViewState["ETAPA"].ToString(), ViewState["SUBETAPA"].ToString(), ViewState["ESTADO"].ToString(), txtBuscar.Text.ToString(), pageSize, pageNro);
            }
        }
            ...
            for (int i = 0; i <= dt.Rows.Count - 1; i++)
            {
                if (dt.Rows[i]["Opcion"].ToString() == e.CommandName)
                {
                    try
                    {
                        ...
                        resumenP.idPAF = int.Parse(ResultadosBusqueda.Rows[index].Cells[14].Text.Trim());
                        resumenP.idEmpresa = int.Parse(ResultadosBusqueda.Rows[index].Cells[0].Text.Trim());
                        resumenP.idOperacion = int.Parse(ResultadosBusqueda.Rows[index].Cells[28].Text.Trim());
                        resumenP.desOperacion = ResultadosBusqueda.Rows[index].Cells[4].Text.Trim();
                        resumenP.desEmpresa = ResultadosBusqueda.Rows[index].Cells[3].Text.Trim();
                        resumenP.descEjecutivo = ResultadosBusqueda.Rows[index].Cells[17].Text.Trim();
                        resumenP.rut = System.Web.HttpUtility.HtmlDecode(ResultadosBusqueda.Rows[index].Cells[9].Text.Trim()) + '-' + System.Web.HttpUtility.HtmlDecode(ResultadosBusqueda.Rows[index].Cells[10].Text.Trim());
                        resumenP.desEstado = System.Web.HttpUtility.HtmlDecode(ResultadosBusqueda.Rows[index].Cells[7].Text.Trim());
                        resumenP.desEtapa = System.Web.HttpUtility.HtmlDecode(ResultadosBusqueda.Rows[index].Cells[5].Text.Trim());
                        resumenP.desSubEtapa = System.Web.HttpUtility.HtmlDecode(ResultadosBusqueda.Rows[index].Cells[6].Text.Trim());
                        ...
                        resumenP.DescEstadoFiscalia = System.Web.HttpUtility.HtmlDecode(ResultadosBusqueda.Rows[index].Cells[19].Text.Trim());
                        resumenP.IdEstadoFiscalia = Convert.ToInt32(System.Web.HttpUtility.HtmlDecode(ResultadosBusqueda.Rows[index].Cells[18].Text.Trim()));
                        resumenP.IdAcreedor = Convert.ToInt32(System.Web.HttpUtility.HtmlDecode(ResultadosBusqueda.Rows[index].Cells[30].Text.Trim()));
                        resumenP.OrdenSubEtapaLegal = Convert.ToInt32(System.Web.HttpUtility.HtmlDecode(ResultadosBusqueda.Rows[index].Cells[32].Text.Trim()));

                        SPWeb app2 = SPContext.Current.Web;
                        resumenP.idUsuario = util.ObtenerValor(app2.CurrentUser.Name);
                        resumenP.idEstado = Convert.ToInt32(System.Web.HttpUtility.HtmlDecode(ResultadosBusqueda.Rows[index].Cells[35].Text.Trim()));
                        resumenP.idEtapa = Convert.ToInt32(System.Web.HttpUtility.HtmlDecode(ResultadosBusqueda.Rows[index].Cells[33].Text.Trim()));
                        resumenP.idSubEtapa = Convert.ToInt32(System.Web.HttpUtility.HtmlDecode(ResultadosBusqueda.Rows[index].Cells[34].Text.Trim()));
        ...
        protected void ResultadosBusqueda_RowCreated(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.Cells.Count > 2)
            {
                ...
                e.Row.Cells[33].Visible = false;//idetapa
                e.Row.Cells[34].Visible = false;//idsubetapa
                e.Row.Cells[35].Visible = false;//idestado
        ...
        protected void ResultadosBusqueda_DataBound(object sender, EventArgs e)
        {
            ...
            {
                IdOperacionTemp = ResultadosBusqueda.Rows[i].Cells[28].Text;
        ...
        public int validarDocumentosSubidos(string idOperacion, string empresa, string rut, string area)
        {
            ...
            dtArchivos = new Documentos { }.buscarArchivos(empresa.Trim(), rut.Trim(), area.Trim(), idOperacion.ToString());
