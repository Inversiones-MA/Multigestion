...
namespace MultiContabilidad
{
  ...
  string Comentario, string NroDocumentoPago, string idBanco, string descBanco, string fechaPago, string idTipoPago, string DescTipoPago, string DescTipoTransaccion)
      ...
      string Etapa, string descEtapa, string subEtapa, string descsubEtapa, string Estado, string descEstado, string Area, string comentario, string id)
        ...
        public Boolean Gestion_BitacoraPago(string xmlBitacora, string NroCertificado, int? IdMotivo, int? IdCausa, int? IdAcreedor, DateTime? FechaCobro, DateTime? FechaPago, string NroDocumento, int? Monto, string Comentario, int? IdBitacoraPago, string IdUsuario, int IdOpcion)
        {
            try
            {
                bool dt = false;
                ...
                SqlParametros[11] = new SqlParameter("@IdUsuario", SqlDbType.NVarChar);
                SqlParametros[11].Value = IdUsuario;
                SqlParametros[12] = new SqlParameter("@IdOpcion", SqlDbType.Int);
                SqlParametros[12].Value = IdOpcion;

                dt = AD_Global.ejecutarAccion("GestionBitacoraPago", SqlParametros);
                return dt;
            }
            catch (Exception ex)
            {
