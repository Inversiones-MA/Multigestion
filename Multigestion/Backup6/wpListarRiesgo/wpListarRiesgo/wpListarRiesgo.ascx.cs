    ...
    public partial class wpListarRiesgo : WebPart
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                if (!Page.IsPostBack)
                {
                    ...
                    {
                        MTO.CargarLista(ref cb_estados, Constantes.LISTAESTADO.ESTADO);
                        MTO.CargarLista(ref cb_etapa, Constantes.LISTAETAPA.ETAPAS);
                        MTO.CargarLista(ref cb_subetapa, Constantes.LISTASUBETAPA.SUBETAPA);
                        ViewState["ESTADO"] = "";
                        ViewState["SUBETAPA"] = "";
                        ViewState["ETAPA"] = "";
        ...
        protected void ResultadosBusqueda_RowCreated(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.Cells.Count > 2)
            {
                ...
                e.Row.Cells[37].Visible = false;  //Bandeja
        ...
        protected void CargarGrid(string etapa, string subEtapa, string estado, string buscar, int pageS, int pageN)
        {
            try
            {
                ...
                LogicaNegocio MTO = new LogicaNegocio();
                DataSet res;
                res = MTO.ListarRiesgo(etapa, subEtapa, estado, buscar, ViewState["AREA"].ToString(), ViewState["CARGO"].ToString(), ViewState["USER"].ToString(), pageS, pageN);
