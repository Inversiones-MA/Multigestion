...
namespace MultiRiesgo.wpPAF.wpPAF
{
    ...
    public partial class wpPAF : WebPart
    {
        ...
        protected void ckSinDocumentos_CheckedChanged(object sender, EventArgs e)
        {
            ...
            MTO.sinDocumentosContables(objresumen.idEmpresa.ToString(), objresumen.idUsuario, objresumen.descCargo, ViewState["idp"].ToString(), ckSinDocumentos.Checked);
        ...
        protected void ActualizarOperacionVigentes_Click(object sender, EventArgs e)
        {
            ...
            asignacionResumen(ref objresumen);
            LogicaNegocio MTO = new LogicaNegocio();
            res = MTO.ObtenerDatosEmpresaPafActual(objresumen.idEmpresa.ToString(), ViewState["idp"].ToString(), objresumen.idUsuario, objresumen.descCargo);
        ...
        protected void btnImprimirPAF2_Click(object sender, EventArgs e)
        {
            ...
            //{
            //    DataSet dt = MTO.ActualizarImpresionPAF(int.Parse(ViewState["idp"].ToString()));
        ...
        protected void btnCalcular_Click(object sender, EventArgs e)
        {
            ...
            LogicaNegocio MTO = new LogicaNegocio();
            ocultarDiv();
            if (MTO.InsertarImpresionPAF(objresumen.idEmpresa, int.Parse(ViewState["idp"].ToString()), objresumen.idUsuario))
            {
        ...
        protected void ConsultarImpresionPAF()
        {
            ...
            //imprimir
            LogicaNegocio MTO = new LogicaNegocio();
            DataSet dt = MTO.ConsultarImpresionPAF(int.Parse(ViewState["idp"].ToString()));
        ...
        protected void btnAdjuntar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (cbEstado.SelectedItem.Text != "--Select--")
                {
                    ...
                    if (LN.GestionPAF(datos, ga, on, ov, objresumen.idUsuario, txtCapacidadPago.InnerText, txtEvaluacionRiesgo.InnerText))
                    {
                        ...
                        if (cbEstado.SelectedItem.Text == "Aprobado")
                            estados = LN.BuscarIdsYValoresCambioDeEstado("9");
                        else if (cbEstado.SelectedItem.Text == "Rechazado")
                            estados = LN.BuscarIdsYValoresCambioDeEstado("10");
                        else
                            estados = LN.BuscarIdsYValoresCambioDeEstado("11");
                        DataTable OperacionesNuevas = (DataTable)ViewState["OperacionesNuevas"];
                ...
                else
                {
                    ...
                    //mensajeError("Datos Imcompletos");
                }
            }
            catch (Exception ex)
            {
        ...
        protected void btnImprimir_Click(object sender, EventArgs e)
        {
            ...
            byte[] archivo = Ln.GenerarReporteOld(Reporte, datos, objresumen.descEjecutivo);
            if (archivo != null)
            {
                Page.Session["tipoDoc"] = "pdf";
                Page.Session["binaryData"] = archivo;
        ...
        public void cargarEmpresa()
        {
            ...
            asignacionResumen(ref objresumen);
            LogicaNegocio MTO = new LogicaNegocio();
            res = MTO.ConsutarDatosEmpresaPAF(objresumen.idEmpresa.ToString(), ViewState["idp"].ToString(), objresumen.idUsuario, objresumen.descCargo);
            if (res.Tables.Count > 0)
            {
        ...
        public void cargaDatosPAF()
        {
            ...
            DataSet res = new DataSet();
            LogicaNegocio MTO = new LogicaNegocio();
            res = MTO.ConsutarDatosPAF(objresumen.idEmpresa.ToString(), objresumen.idUsuario, objresumen.descCargo, objresumen.idOperacion.ToString());
            ...
            if (res.Tables[0].Rows.Count > 0)
            {
                ...
                if (string.IsNullOrEmpty(res.Tables[0].Rows[0]["Fecha"].ToString()))
                {
                    try
                    {
                        string indicador = MTO.BuscarValorIndicador(DateTime.Now);
                        ...
                        {
                            txtDOLAR.Text = indicador.Substring(0, indicador.IndexOf("/"));
                            txtUF.Text = indicador.Substring(indicador.IndexOf("/") + 1);
                        }

                        txtDOLARV.Text = Convert.ToDouble(txtDOLAR.Text).ToString("N3", CultureInfo.CreateSpecificCulture("es-ES"));
                        txtUFV.Text = Convert.ToDouble(txtUF.Text).ToString("N3", CultureInfo.CreateSpecificCulture("es-ES")); ;
                    }
                    catch (Exception ex)
                    {
                        LoggingError.PostEventRegister(ex, ConfigurationManager.AppSettings["pathLog"].ToString(), "", "", ConfigurationManager.AppSettings["logName"].ToString(), Convert.ToBoolean(ConfigurationManager.AppSettings["enabledLog"].ToString()), Convert.ToBoolean(ConfigurationManager.AppSettings["enabledEventViewer"].ToString()), ConfigurationManager.AppSettings["registerEventsTypes"].ToString(), EventLogEntryType.Error);
                ...
                else
                {
                    ...
                    mensajeAlerta(lbWarning.Text + MTO.buscarMensaje(26) + " Balance incompleto.");
                }
            }
            else
            {
                mensajeAlerta(lbWarning.Text + MTO.buscarMensaje(26) + " Balance, Estado Resultado, IVA Compras, IVA Ventas ó Scoring Incompleto(s).");
            }
        }
        ...
        {
            LogicaNegocio MTO = new LogicaNegocio();
            MTO.ListarUsuarios(ref cbUsuarios1);
            MTO.ListarUsuarios(ref cbUsuarios2);
            MTO.ListarUsuarios(ref cbUsuarios3);
            MTO.ListarUsuarios(ref cbUsuarios4);
            MTO.ListarUsuarios(ref cbUsuarios5);
        }
        ...
        public void guardarArchivo()
        {
            ...
            bool cargaCorrecta = util.cargarDocumento(carpetaEmpresa, objresumen.area.ToUpper(), objresumen.idOperacion.ToString(), objresumen, fileDocumento, "PAF", objresumen.idOperacion.ToString(), "Documento adjunto PAF");
        }
            ...
            try
            {
                if (ValidarMontoPropuesto())
                {
                    if (ValidarCheckSeguros())
                    {
                        ...
                        if (MTO.GestionPAF(datos, ga, on, ov, objresumen.idUsuario, txtCapacidadPago.InnerText, txtEvaluacionRiesgo.InnerText))
                        {
                            datos.Clear();
                            datos.Add("IdPaf", ViewState["idp"].ToString());
                            datos.Add("IdSP1", cbUsuarios1.SelectedValue);
                            ...
                            datos.Add("IdSP5", cbUsuarios5.SelectedValue);
                            datos.Add("Nombre5", cbUsuarios5.SelectedItem.Text);
                            MTO.RegistrarAprobadoresPAF(datos, cbEstado.SelectedValue.ToString(), dtcFecResolucion.SelectedDate.ToString());
                            mensajeExito(MTO.buscarMensaje(4));
                        }
                        else
                        {
                            mensajeAlerta(MTO.buscarMensaje(5));
                        }
                    }
                    else
