    ...
    public partial class wpVaciadoEstadoResultado : WebPart
    {
        ...
        protected void ResultadosBusqueda_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.RowType == DataControlRowType.Header)
            {
                ...
                MTO.CargarListaEstadoResultado(ref cmbTipoDocumentAux);
        ...
        protected void btnFinalizar_Click(object sender, EventArgs e)
        {
            ...
            if (mensaje == string.Empty)//MENSAJE ME INDICA QUE HACE FALTA GUARDAR ALGUNOS AÑOS
            {
                ...
                {
                    exito = MTO.finalizarEstadoResultado(objresumen.idEmpresa.ToString(), System.Web.HttpUtility.HtmlDecode(ResultadosBusqueda.Rows[0].Cells[14].Text.ToString()).Trim(), xmlData, Constantes.RIESGO.VALIDACION);
                    pnFormulario.Enabled = false;
                    if (exito)
                    {
                        ocultarDiv();
                ...
                else
                {
                    ...
                    exito = MTO.InsertarFinalizarEstado(objresumen.idEmpresa.ToString(), xmlData);
                    pnFormulario.Enabled = false;
                    if (exito)
                    {
                        ocultarDiv();
        ...
        private void llenargrid()
        {
            ...
            LogicaNegocio MTO = new LogicaNegocio();
            DataTable res;
            res = MTO.ListarEstadoResultado(objresumen.idEmpresa.ToString());
            int con = 0;
        ...
        protected void grabar()
        {
            ...
            if ((auditado && ComentarioExiste) || (!auditado))
            {
                if (permiteGuardar)
                {
                    ...
                    {
                        exito = MTO.InsertarEstadoResultado(objresumen.idEmpresa.ToString(), xmlDatos, objresumen.idUsuario, txtComentario.Text);
                        if (exito)
                        {
                            ocultarDiv();
                            dvSuccess.Style.Add("display", "block");
                    ...
                    {
                        exito = MTO.ModificarEstadoResultado(objresumen.idEmpresa.ToString(), System.Web.HttpUtility.HtmlDecode(ResultadosBusqueda.Rows[0].Cells[14].Text.ToString()).Trim(), xmlDatos, objresumen.idUsuario, txtComentario.Text);
                        if (exito)
                        {
                            ocultarDiv();
                            dvSuccess.Style.Add("display", "block");
